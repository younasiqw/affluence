...
public:
	...
	template <typename Fn = void*> inline const Fn getoriginal(std::size_t function_index)
	{
		return reinterpret_cast<Fn>(this->original_vft[function_index]);
	}

	inline bool hook(void* new_function, const std::size_t function_index)
	{
	...
	inline bool unhook(const std::size_t function_index) 
	{
		if (function_index > this->total_functions)
			return false;
